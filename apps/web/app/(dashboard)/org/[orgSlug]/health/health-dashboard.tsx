...
export function HealthDashboard({
  ...
  const totalPriorityCount =
    priorityBuckets.high +
    priorityBuckets.medium +
    priorityBuckets.low +
    priorityBuckets.none;
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          <div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
            {checks.map((check) => (
              <div
                key={check.name}
                className={`flex items-center gap-3 rounded-lg border px-3 py-2.5 ${statusColors[check.status]}`}
              >
                {statusIcons[check.status]}
                <div className="min-w-0 flex-1">
                  <div className="font-mono text-[11px] font-medium text-[var(--landing-text)]">
                    {check.name}
                  </div>
                  <div className="font-mono text-[10px] text-[var(--landing-text-tertiary)]">
                    {check.detail}
                  </div>
                </div>
              </div>
          ...
          {[
            {
              icon: Brain,
              label: "Active",
              value: stats.totalMemories - stats.totalArchived,
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="dash-card p-4">
            <div className="mb-3 font-mono text-xs font-medium text-[var(--landing-text)]">
              Priority Distribution
            </div>
            <div className="space-y-2">
              {[
                {
                  label: "High (70-100)",
                  value: priorityBuckets.high,
                  color: "bg-[#F97316]",
                },
                {
                  label: "Medium (30-69)",
                  value: priorityBuckets.medium,
                  color: "bg-amber-500",
                },
                {
                  label: "Low (1-29)",
                  value: priorityBuckets.low,
                  color: "bg-blue-500",
                },
                {
                  label: "None (0)",
                  value: priorityBuckets.none,
                  color: "bg-[var(--landing-text-tertiary)]",
                },
              ].map(({ label, value, color }) => {
                const pct =
                  totalPriorityCount > 0
                    ? Math.round((value / totalPriorityCount) * 100)
                    : 0;
                return (
                  <div key={label} className="flex items-center gap-3">
                    <span className="w-24 shrink-0 font-mono text-[10px] text-[var(--landing-text-secondary)]">
                      {label}
                    </span>
                    <div className="h-2 flex-1 overflow-hidden rounded-full bg-[var(--landing-surface-2)]">
                      <div
                        className={`h-full rounded-full ${color} transition-all duration-500`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                    <span className="w-12 text-right font-mono text-[10px] text-[var(--landing-text-tertiary)]">
                      {value} ({pct}%)
                    </span>
                  </div>
                );
                ...
                {topTags.map(({ tag, count }) => (
                  <Tooltip key={tag}>
                    <TooltipTrigger asChild>
                      <span className="inline-flex cursor-default items-center gap-1 rounded-md bg-[var(--landing-surface-2)] px-2 py-1 font-mono text-[10px] text-[var(--landing-text-secondary)] transition-colors hover:text-[var(--landing-text)]">
                        <Tag className="h-2.5 w-2.5 text-[#F97316]" />
                        {tag}
                    ...
                    <TooltipContent>
                      <p className="text-xs">
                        {count} memor{count === 1 ? "y" : "ies"} tagged &quot;
                        {tag}&quot;
                      </p>
                    </TooltipContent>
                  </Tooltip>
        ...
        <div className="dash-card mt-4 overflow-hidden">
          <div className="border-b border-[var(--landing-border)] bg-[var(--landing-code-bg)] px-4 py-2.5">
            <span className="font-mono text-xs font-medium text-[var(--landing-text)]">
              Project Breakdown
            </span>
          </div>
                ...
                {projectStats.map((p) => {
                  const share =
                    stats.totalMemories > 0
                      ? Math.round((p.memoryCount / stats.totalMemories) * 100)
                      : 0;
                  return (
                    <TableRow
                      key={p.slug}
                    ...
                    >
                      ...
                      <TableCell className="text-right font-mono text-[11px] text-[var(--landing-text)]">
                        {p.memoryCount}
                      </TableCell>
                      <TableCell className="text-right font-mono text-[11px] text-[#F97316]">
                        {p.pinnedCount}
                      </TableCell>
                      <TableCell className="text-right font-mono text-[11px] text-[var(--landing-text-tertiary)]">
                        {p.archivedCount}
                      </TableCell>
                      <TableCell className="text-right font-mono text-[11px] text-blue-400">
                        {p.sessionCount}
                      </TableCell>
                      <TableCell className="text-right font-mono text-[11px] text-emerald-400">
                        {p.activityCount}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1.5">
                          <div className="h-1.5 w-16 overflow-hidden rounded-full bg-[var(--landing-surface-2)]">
                            <div
