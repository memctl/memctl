---
title: AGENTS.md
description: Copy-paste AGENTS.md template for mem/ctl MCP usage
---

# AGENTS.md

Copy this into your repository root as `AGENTS.md`. This works with any MCP-compatible agent (Claude Code, Cursor, VS Code, Windsurf, Cline, etc.).

Some agents read a different filename. Symlink instead of duplicating:

```bash
# Claude Code reads CLAUDE.md
ln -s AGENTS.md CLAUDE.md

# Google Gemini reads GEMINI.md
ln -s AGENTS.md GEMINI.md
```

```md
# AGENTS.md

## memctl MCP Rules

MCP server name is `memctl`.
Use `memctl` for all project memory: session history, coding conventions, architecture decisions, testing rules, branch plans.
Store decisions and lessons learned in memctl so they persist across sessions.

## Session Start

1. `context.bootstrap` - load all project context
2. `session.start` with `autoExtractGit=true` - register this session
3. `activity.memo_read` - check for handoff notes from the last session
4. `branch.get` - load current branch plan if one exists

If the file map is stale, run `repo.scan` with `saveAsContext=true`.

## Before Coding

- `context.functionality_get` for: `coding_style`, `architecture`, `testing`, `constraints`, `workflow`
- `context.context_for` for files you are about to touch
- `branch.get` for current branch plan and checklist

## While Coding

- `context.functionality_set` for structured project rules
- `memory.store` for one-off decisions or notes (search first with `memory.search` to avoid duplicates)
- `branch.set` after each meaningful task to save progress

## End Of Session

1. `branch.set` with final progress
2. `activity.memo_leave` if anything is pending
3. `session.end` with a summary of what was done

## Maintenance

Run every few sessions:

- `memory_lifecycle.suggest_cleanup` - review stale candidates
- `memory_lifecycle.lifecycle_run` - clean expired, session logs, merged branches
- `memory_advanced.check_duplicates` - find and merge duplicates

## Rules

- Do not store secrets, tokens, or API keys
- Do not store large file contents or binary data
- Do not skip `context.bootstrap` at session start
- Search before storing to avoid duplicates
- Do not let memory grow unchecked
```

## Importing and exporting

You can generate this file from your stored context:

```bash
# Export stored context as AGENTS.md
npx memctl export agents_md > AGENTS.md

# Import an existing AGENTS.md into memctl
npx memctl import AGENTS.md

# Export as .cursorrules for Cursor
npx memctl export cursorrules > .cursorrules
```

Or via MCP tools:

```
import_export.export_agents_md   - generate AGENTS.md from stored context
import_export.agents_md_import   - parse and import AGENTS.md content (supports dryRun)
import_export.cursorrules_import - parse and import .cursorrules content
```

Need full tool details? See [MCP Tools](/docs/mcp-server/tools).
