...
npx memctl generate --windsurf         # + .windsurfrules
npx memctl generate --cline            # + .clinerules
...
With `--link`, files that share the same format as `AGENTS.md` (CLAUDE.md, GEMINI.md, copilot-instructions.md, .windsurfrules, .clinerules, .roo/rules/memctl.md) become symlinks. `.cursorrules` is always a real file because it uses a different format. See [generate docs](/docs/cli/import-export#generate) for details.
