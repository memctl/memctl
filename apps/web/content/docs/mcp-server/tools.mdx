...
Write operations (`store`, `delete`, `update`) are [rate-limited](/docs/concepts#rate-limiting).
...
Context entries are stored as memories with keys following the pattern `agent/context/{type}/{id}`. See [Concepts](/docs/concepts#context-entries) for details.
...
Session tracking and agent coordination. See [Concepts](/docs/concepts#sessions) for how sessions enable handoff between conversations.
...
| Parameter        | Used by             | Description                                           |
| ---------------- | ------------------- | ----------------------------------------------------- |
| `autoExtractGit` | start               | Extract branch, commits, and diffs since last session |
