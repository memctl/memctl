...
description: Typed context entries, bootstrapping, token budgeting, and smart retrieval
---

# Context System

mem/ctl organizes agent knowledge into typed context entries. Each entry has a type, an ID, and content. The context system provides bootstrapping, file-aware retrieval, token budgeting, and import/export.
...
Branch plans use a different format: `branch_plan:{branch_name}`.

## Creating context entries

Use the `context` tool with the `functionality_set` action:
...
Each built-in type has a markdown template. Get it with the `context_config` tool's `template_get` action:
...
Returns a structured template with sections for language conventions, naming, formatting, and patterns. Custom types get a generic template.
...
### Delta and compact bootstrap

For subsequent calls, use `bootstrap_delta` with a timestamp to get only changes since the last bootstrap:

```json
{
  "action": "bootstrap_delta",
...
Custom types work exactly like built-in types. They can have entries, be included in bootstrap, and participate in all context operations. You cannot override or delete built-in types.
...
Find conflicting directives across context entries:
...
Available formats: `agents_md`, `cursorrules`, `json`. See [AGENTS.md Integration](/docs/agents-md) for details on the import/export workflow.
