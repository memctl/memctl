...
export function registerRepoTool(server: McpServer, client: ApiClient, _rl: RateLimitState) {
  server.tool(
    "repo",
    "Repository operations. Actions: scan, scan_check, onboard",
    {
      action: z.enum(["scan", "scan_check", "onboard"]).describe("Which operation to perform"),
      maxFiles: z.number().int().min(10).max(5000).optional().describe("[scan] Max files to include"),
      includePatterns: z.array(z.string()).optional().describe("[scan] Glob patterns to include"),
      excludePatterns: z.array(z.string()).optional().describe("[scan] Glob patterns to exclude"),
      saveAsContext: z.boolean().optional().describe("[scan] Save result as context entry"),
      apply: z.boolean().optional().describe("[onboard] Store generated memories"),
    },
    async (params) => {
      try {
        switch (params.action) {
          case "scan": {
            ...
            const result = await execFileAsync("git", args, { cwd: process.cwd(), maxBuffer: 10 * 1024 * 1024 });
            let files = result.stdout.trim().split("\n").filter(Boolean);
            if (params.includePatterns?.length) files = files.filter((f) => params.includePatterns!.some((p) => matchGlob(f, p)));
            if (params.excludePatterns?.length) files = files.filter((f) => !params.excludePatterns!.some((p) => matchGlob(f, p)));
            ...
            const fileMap = {
              ...
              directories: Object.entries(byDir).sort((a, b) => b[1].length - a[1].length).map(([dir, dirFiles]) => ({ directory: dir, fileCount: dirFiles.length, files: dirFiles.slice(0, 50), truncated: dirFiles.length > 50 })),
              extensionBreakdown: Object.entries(byExt).sort((a, b) => b[1] - a[1]).map(([ext, count]) => ({ extension: ext, count })),
            };

            if (params.saveAsContext) {
              const content = JSON.stringify(fileMap, null, 2);
              const key = buildAgentContextKey("file_map", "auto-scan");
              await client.storeMemory(key, content, { scope: "agent_functionality", type: "file_map", id: "auto-scan", title: "Auto-scanned file map", updatedByTool: "repo.scan", updatedAt: new Date().toISOString() });
              return textResponse(`Repository scanned: ${files.length} files. Saved as ${key}.`);
            }
            return textResponse(JSON.stringify(fileMap, null, 2));
          }
          case "scan_check": {
            const result = await execFileAsync("git", ["ls-files", "--cached", "--others", "--exclude-standard"], { cwd: process.cwd(), maxBuffer: 10 * 1024 * 1024 });
            ...
            try {
              const mem = await client.getMemory(key) as { memory?: { content?: string } };
              if (mem?.memory?.content) storedMap = JSON.parse(mem.memory.content);
            } catch {
              return textResponse("No stored file_map found. Run repo scan first.");
            }
            const storedFiles = new Set<string>();
            if (storedMap?.directories) { for (const dir of storedMap.directories) { if (dir.files) { for (const f of dir.files) storedFiles.add(f); } } }
            ...
            return textResponse(JSON.stringify({
              isStale, currentFileCount: currentFiles.size, storedFileCount: storedFiles.size,
              newFiles: newFiles.slice(0, 50), deletedFiles: deletedFiles.slice(0, 50),
              newFilesTotal: newFiles.length, deletedFilesTotal: deletedFiles.length,
              recommendation: isStale ? "Run repo scan with saveAsContext=true to update." : "File map is up to date.",
            }, null, 2));
          }
          case "onboard":
...
async function handleOnboard(client: ApiClient, apply: boolean) {
  ...
  async function readFile(path: string): Promise<string | null> {
    try { const result = await execFileAsync("cat", [path], { cwd }); return result.stdout; } catch { return null; }
  }
  ...
  if (pkgJson) {
    try {
      ...
      if (deps.typescript) detected.language = "TypeScript"; else detected.language = "JavaScript";
  ...
  if (detected.language || detected.framework) {
    ...
    suggestions.push({ type: "coding_style", id: "project-stack", title: "Project Stack & Conventions", content: `## Tech Stack\n${parts.join("\n")}`, priority: 80 });
  }
  if (detected.testRunner) suggestions.push({ type: "testing", id: "test-setup", title: "Test Setup", content: `## Test Runner\n- Runner: ${detected.testRunner}\n- Run: \`${detected.packageManager} ${detected.packageManager === "npm" ? "run " : ""}test\``, priority: 70 });
  if (detected.framework) suggestions.push({ type: "architecture", id: "framework-overview", title: "Architecture Overview", content: `## Framework\n- ${detected.framework}${detected.monorepo ? `\n- Monorepo: ${detected.monorepo}` : ""}${detected.docker ? "\n- Containerized with Docker" : ""}`, priority: 75 });
  if (detected.ci) suggestions.push({ type: "workflow", id: "ci-cd", title: "CI/CD", content: `## CI/CD\n- Platform: ${detected.ci}`, priority: 60 });
  ...
  if (apply && suggestions.length > 0) {
    ...
    for (const s of suggestions) {
      ...
      try {
        await client.storeMemory(key, s.content, { scope: "agent_functionality", type: s.type, id: s.id, title: s.title, updatedByTool: "repo.onboard", updatedAt: new Date().toISOString() }, { priority: s.priority });
        results.push({ key, status: "stored" });
      } catch (err) {
        results.push({ key, status: `error: ${err instanceof Error ? err.message : String(err)}` });
      }
    }
    return textResponse(JSON.stringify({ detected, applied: true, stored: results.filter((r) => r.status === "stored").length, errors: results.filter((r) => r.status.startsWith("error")).length, details: results }, null, 2));
  }

  return textResponse(JSON.stringify({
    detected, suggestions: suggestions.map((s) => ({ type: s.type, id: s.id, title: s.title, key: buildAgentContextKey(s.type, s.id), priority: s.priority, contentPreview: s.content.slice(0, 200) })),
    hint: suggestions.length > 0 ? `Found ${suggestions.length} suggestions. Call again with apply=true to store.` : "No configs detected.",
  }, null, 2));
}
