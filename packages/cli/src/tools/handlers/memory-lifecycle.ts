...
import { listAllMemories } from "../../agent-context.js";
...
export function registerMemoryLifecycleTool(server: McpServer, client: ApiClient, rl: RateLimitState) {
  server.tool(
    ...
    "Memory lifecycle management. Actions: cleanup, suggest_cleanup, lifecycle_run, lifecycle_schedule, validate_references, prune_stale, feedback, analytics, lock, unlock, health. Policies for lifecycle_run: archive_merged_branches, cleanup_expired, cleanup_session_logs, auto_promote, auto_demote, auto_prune, auto_archive_unhealthy, cleanup_old_versions, cleanup_activity_logs, cleanup_expired_locks, purge_archived",
    {
      ...
      policies: z.array(z.enum(["archive_merged_branches","cleanup_expired","cleanup_session_logs","auto_promote","auto_demote","auto_prune","auto_archive_unhealthy","cleanup_old_versions","cleanup_activity_logs","cleanup_expired_locks","purge_archived"])).optional().describe("[lifecycle_run] Policies to run"),
      ...
      try {
        switch (params.action) {
          ...
          case "lifecycle_schedule": {
            ...
            const totalAffected = Object.values(result.results).reduce((sum, r) => sum + r.affected, 0);
            return textResponse(JSON.stringify({
              ranAt: result.ranAt, totalAffected, policies: result.results,
              message: totalAffected > 0 ? `Lifecycle complete: ${totalAffected} entries affected.` : "Lifecycle complete: no entries needed maintenance.",
            }, null, 2));
          }
          case "validate_references": {
            const gitResult = await execFileAsync("git", ["ls-files", "--cached", "--others", "--exclude-standard"], { cwd: process.cwd(), maxBuffer: 10 * 1024 * 1024 });
            const repoFiles = gitResult.stdout.trim().split("\n").filter(Boolean);
            const validation = await client.validateReferences(repoFiles);
            if (validation.issuesFound === 0) return textResponse(`All references valid. Checked ${validation.totalMemoriesChecked} memories against ${repoFiles.length} repo files.`);
            return textResponse(JSON.stringify({ summary: `Found ${validation.issuesFound} memories with stale file references.`, ...validation }, null, 2));
          }
          case "prune_stale": {
            const gitResult = await execFileAsync("git", ["ls-files", "--cached", "--others", "--exclude-standard"], { cwd: process.cwd(), maxBuffer: 10 * 1024 * 1024 });
            const repoFiles = gitResult.stdout.trim().split("\n").filter(Boolean);
            const validation = await client.validateReferences(repoFiles);
            if (validation.issuesFound === 0) return textResponse("No stale references found.");
            if (params.archiveStale) {
              ...
              return textResponse(`Archived ${batchResult.affected} memories with stale file references.`);
            }
            return textResponse(JSON.stringify({ summary: `${validation.issuesFound} memories reference deleted files.`, issues: validation.issues, hint: "Call again with archiveStale=true to archive." }, null, 2));
          }
          case "feedback": {
            if (!params.key) return errorResponse("Missing param", "key required");
            if (params.helpful === undefined) return errorResponse("Missing param", "helpful required");
            const result = await client.feedbackMemory(params.key, params.helpful);
            return textResponse(`Feedback recorded for "${params.key}": ${result.feedback} (${result.helpfulCount} helpful, ${result.unhelpfulCount} unhelpful)`);
          }
          case "analytics": {
          ...
          case "lock": {
            if (!params.key) return errorResponse("Missing param", "key required");
            const result = await client.lockMemory(params.key, params.lockedBy, params.ttlSeconds ?? 60);
            if (!result.acquired) return textResponse(JSON.stringify({ acquired: false, key: params.key, message: "Lock held by another agent.", currentLock: result.lock }, null, 2));
            return textResponse(JSON.stringify({ acquired: true, key: params.key, expiresAt: result.lock.expiresAt, message: `Lock acquired. Expires in ${params.ttlSeconds ?? 60}s.` }, null, 2));
          }
          case "unlock": {
            if (!params.key) return errorResponse("Missing param", "key required");
            const result = await client.unlockMemory(params.key, params.lockedBy);
            return textResponse(`Lock released for key: ${params.key} (released: ${result.released})`);
          }
          case "health": {
        ...
        return errorResponse(`Error in memory_lifecycle.${params.action}`, error);
      }
    },
  );
}
